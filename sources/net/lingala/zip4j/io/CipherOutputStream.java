...
public class CipherOutputStream extends BaseOutputStream {
    ...
    public void closeEntry() throws IOException, ZipException {
        ...
        if (this.fileHeader.isEncrypted()) {
            if (this.fileHeader.getEncryptionMethod() == 99) {
                value = 0;
            } else if (this.fileHeader.getEncryptionMethod() == 0 && ((int) value) != this.zipParameters.getSourceFileCRC()) {
        ...
        if (this.outputStream instanceof SplitOutputStream) {
            ...
            if (this.localFileHeader.isWriteComprSizeInZip64ExtraRecord()) {
                ...
                Raw.writeLongLittleEndian(bArr2, 0, this.localFileHeader.getCompressedSize());
                headerWriter.updateLocalFileHeader(this.localFileHeader, this.fileHeader.getOffsetLocalHeader(), 18, this.zipModel, bArr2, this.fileHeader.getDiskNumberStart(), (SplitOutputStream) this.outputStream);
            } else {
                Raw.writeIntLittleEndian(bArr, 0, (int) this.localFileHeader.getCompressedSize());
                headerWriter.updateLocalFileHeader(this.localFileHeader, this.fileHeader.getOffsetLocalHeader(), 18, this.zipModel, bArr, this.fileHeader.getDiskNumberStart(), (SplitOutputStream) this.outputStream);
            }
            if (this.localFileHeader.getCrc32() != 0) {
                Raw.writeIntLittleEndian(bArr, 0, (int) this.localFileHeader.getCrc32());
                headerWriter.updateLocalFileHeader(this.localFileHeader, this.fileHeader.getOffsetLocalHeader(), 14, this.zipModel, bArr, this.fileHeader.getDiskNumberStart(), (SplitOutputStream) this.outputStream);
            }
        } else {
            this.totalBytesWritten += headerWriter.writeExtendedLocalHeader(this.localFileHeader, r3);
        }
        this.crc.reset();
        this.bytesWrittenForThisFile = 0L;
        this.encrypter = null;
    ...
    public void finish() throws IOException, ZipException {
        ...
        if (this.outputStream instanceof SplitOutputStream) {
            ...
            i = ((SplitOutputStream) this.outputStream).getCurrSplitFileCounter();
        } else {
        ...
        if (this.zipModel.isZip64Format()) {
            ...
            this.zipModel.getZip64EndCentralDirLocator().setNoOfDiskStartOfZip64EndOfCentralDirRec(i);
            this.zipModel.getZip64EndCentralDirLocator().setTotNumberOfDiscs(i + 1);
        }
        this.zipModel.getEndCentralDirRecord().setNoOfThisDisk(i);
        this.zipModel.getEndCentralDirRecord().setNoOfThisDiskStartOfCentralDir(i);
    ...
    private void createFileHeader() throws ZipException {
        ...
        if ((Zip4jUtil.isStringNotNullAndNotEmpty(this.zipModel.getFileNameCharset()) && this.zipModel.getFileNameCharset().equalsIgnoreCase(InternalZipConstants.CHARSET_UTF8)) || Zip4jUtil.detectCharSet(this.fileHeader.getFileName()).equals(InternalZipConstants.CHARSET_UTF8)) {
            bArr[1] = 8;
        } else {
            bArr[1] = 0;
        }
