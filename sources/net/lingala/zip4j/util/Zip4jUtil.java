...
public class Zip4jUtil {
    ...
    public static long getLastModifiedFileTime(File file, TimeZone timeZone) throws ZipException {
        ...
        return getDayLightSavingsTime(timeZone, file.lastModified());
    }

    private static long getDayLightSavingsTime(TimeZone timeZone, long j) throws ZipException {
        if (timeZone == null) {
            try {
    ...
    public static long getLocalTimeFromDSTTime(TimeZone timeZone, long j) throws ZipException {
        if (timeZone == null) {
            try {
                timeZone = TimeZone.getDefault();
            } catch (Exception unused) {
    ...
    public static long dosToJavaTme(int i) {
        ...
        int i5 = (i >> 16) & 31;
        int i6 = ((i >> 21) & 15) - 1;
        int i7 = ((i >> 25) & WorkQueueKt.MASK) + 1980;
        Calendar calendar = Calendar.getInstance();
    ...
    public static ArrayList getSplitZipFiles(ZipModel zipModel) throws ZipException {
        ...
        for (int i = 0; i <= noOfThisDisk; i++) {
            if (i == noOfThisDisk) {
                ...
                arrayList.add(i >= 9 ? zipFile.substring(0, zipFile.lastIndexOf(".")) + ".z" + (i + 1) : zipFile.substring(0, zipFile.lastIndexOf(".")) + ".z0" + (i + 1));
            }
        }
        return arrayList;
    }
    ...
    public static String getRelativeFileName(String str, String str2, String str3) throws ZipException {
        ...
        if (isStringNotNullAndNotEmpty(str3)) {
            ...
            if (file.isDirectory()) {
                ...
                fileNameFromFilePath = substring.substring(0, substring.indexOf(file.getName())).replaceAll("\\\\", InternalZipConstants.ZIP_FILE_SEPARATOR) + file.getName();
            }
        } else {
