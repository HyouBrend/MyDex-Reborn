            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m1886constructorimpl = SegmentOrClosed.m1886constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1886constructorimpl = SegmentOrClosed.m1886constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1891isClosedimpl(m1886constructorimpl)) {
                break;
            }
            Segment m1889getSegmentimpl = SegmentOrClosed.m1889getSegmentimpl(m1886constructorimpl);
            while (true) {
                Segment segment4 = (Segment) this.tail;
                if (segment4.getId() >= m1889getSegmentimpl.getId()) {
                    break;
                }
                if (!m1889getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment4, m1889getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m1889getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m1889getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1889getSegmentimpl(m1886constructorimpl);
