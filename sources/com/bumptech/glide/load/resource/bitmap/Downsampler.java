        ...
        if ((options.inSampleSize == 1 || z5) && shouldUsePool(imageType)) {
            if (i6 < 0 || i7 < 0 || !z2 || !z5) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + round + "x" + round2 + "] for source [" + i6 + "x" + i7 + "], sampleSize: " + i8 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                round = i4;
                round2 = i5;
